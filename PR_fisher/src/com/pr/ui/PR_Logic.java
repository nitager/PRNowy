...
import Jama.Matrix;
import com.pr.stat.ClassCoordinates;
...
public class PR_Logic {
    ...
    protected List<ClassCoordinates> ClassIndexList;
    ...
    public void separeteClassObjectFromF() {
        ClassIndexList = new ArrayList<ClassCoordinates>();
        objectsSet = new Matrix(features);
 //           System.out.println("Samples :" );
        //          PRtools.printArray(classLabels);
        int last = 0, temp, actual, end, start = 0;
        int lastStart = 0, lastEnd = 0;
        ClassCoordinates first = new ClassCoordinates();
        ...
        for (int i = 0; i < classLabels.length; i++) {
            ...
            lastEnd = i;
            //    System.out.println("I : " +i + " Actual :" + actual + " Last :" + last);
            if (last != actual) {
              //detekcja pierwszej grupy obiektó klas danej grupy, czyli zawaroość
            //    classlabels zmienia sie, bo zmienia się etykiet klasy obiektów
                
                if (firstChanged == false) {
                    //wykrywanie indeksów skrajnych pierwszej klasy
                    first.setStart(lastStart);
                    first.setEnd(lastEnd - 1);
                    firstChanged = true;
                    lastStart = i;
                    ClassIndexList.add(first);
                    System.out.println("Koordynaty obiektów pierwszej klasy : ");
                    System.out.println("Indeks pocz : " + first.getStart() + " Indeks końcowy :" + first.getEnd());
                }
                if (firstChanged == true) {
//                    if (lastEnd == classLabels.length - 1) {
//                        //detekcja ostatniego rozdziału
//                       ClassCoordinates tempCC = new ClassCoordinates(lastStart, lastEnd);
//                        ClassIndexList.add(tempCC);
//                        System.out.println("Koordynaty obiektów ostatniej klasy : ");
//                        System.out.println("Indeks pocz : " + tempCC.getStart() + " Indeks końcowy :" + tempCC.getEnd());
//                        lastStart=i;
//                    } else {
//                     
                        
                        ClassCoordinates tempCC = new ClassCoordinates(lastStart, lastEnd-1);
                        ClassIndexList.add(tempCC);
                        System.out.println("Koordynaty obiektów kolejnej klasy : ");
                        System.out.println("Indeks pocz : " + tempCC.getStart() + " Indeks końcowy :" + tempCC.getEnd());
                        lastStart = i;
//                    }
                }
            }
          System.out.println("I : " +i + " Start :" + lastStart + " End :" + lastEnd);
            last = actual;
        }
